...
Confirm that `zai/glm-5.3` at `max` is configured and authenticated, and confirm each selected subagent model before dispatch. Missing access is a blocker, not permission to silently select another model. The GLM coordinator fallback is not a full OpenAI-provider fallback and cannot substitute for an unavailable OpenAI-powered specialist.
